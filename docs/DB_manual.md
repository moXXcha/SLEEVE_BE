...
### 手順 4: ent/schema の更新とコード生成

承認された変更計画に基づき、以下の手順で実装を進めます。各ステップ毎にコミットしてください。

#### 4-1. ent/schema の更新 (commit)

`ent/schema/` ディレクトリ内の該当するスキーマファイルを変更、または新規に作成します。

**コミットメッセージ例**: `feat: Add profile_image_url column to User schema`

#### 4-2. task ent-gen でコード生成 (commit)

```bash
task ent-gen
```

entのスキーマ定義からGoのコードを自動生成します。

**コミットメッセージ例**: `chore: Generate ent code for User schema changes`

#### 4-3. task sql-gen で SQL 生成 (commit)

```bash
task sql-gen
```

entスキーマからマイグレーション用のSQLファイルを生成します。生成されたSQLファイルは `migrate/` ディレクトリに配置されます。

**コミットメッセージ例**: `chore: Generate migration SQL for User schema changes`

#### 4-4. task migrate-up でマイグレーション実行
...
**注意**: この手順は通常コミットしません。マイグレーション実行は開発環境や本番環境で行うオペレーションです。
...
マイグレーション実行後に問題が発生した場合、以下の方法でロールバックできます。

#### 6-1. task migrate-rollback による1件ロールバック
...
**重要な制限事項:**
- **最新の1件のみロールバック可能**: 直前に適用されたマイグレーションファイル1つだけを取り消します
- **複数件のまとめてロールバックは不可**: 1回の実行で複数のマイグレーションは戻せません
- **バージョン指定は不可**: 特定のバージョンを指定してロールバックすることはできません

#### 6-2. 複数のマイグレーションをロールバックしたい場合

複数のマイグレーションを戻したい場合は、以下の手順で対応してください：

1. `ent/schema/` のスキーマファイルを手動で編集し、目的の状態に戻す
2. `task sql-gen` で新しいマイグレーションファイルを生成
3. `task migrate-up` でマイグレーションを適用

**例**: 3つ前の状態に戻したい場合
- ❌ 悪い方法: `task migrate-rollback` を3回実行（推奨しません）
- ✅ 良い方法: `ent/schema/` を3つ前の状態に編集 → `task sql-gen` → `task migrate-up`

#### 6-3. ロールバック時の注意点

- **データ損失のリスク**: ロールバックはテーブルやカラムを削除する可能性があるため、本番環境では慎重に実行してください
- **開発環境での確認**: 本番環境で実行する前に、必ず開発環境で動作を確認してください
- **バックアップ**: 重要なデータがある場合は、ロールバック前にバックアップを取得してください

---

## 4. 変更計画書テンプレート (`table_edit_plan.md`)

***docs/templates/table_edit_plan.mdを参照してください***
